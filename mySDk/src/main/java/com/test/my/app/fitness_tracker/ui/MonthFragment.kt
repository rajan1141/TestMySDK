...
import com.test.my.app.databinding.FragmentMonthBinding
import com.test.my.app.fitness_tracker.adapter.DailyDataAdapter
import com.test.my.app.fitness_tracker.adapter.MonthlyAdapter
...
@AndroidEntryPoint
class MonthFragment : BaseFragment(), MonthlyAdapter.OnMonthlyItemClickListener {
    ...
    private var monthlyAdapter: MonthlyAdapter? = null
    ...
    private fun initialise() {
        ...
        val monthlyList = fitnessDataActivity!!.monthlyList

        binding.graphMonthly.visibility = View.INVISIBLE
        ...
        binding.rvWeekMonth.layoutManager!!.scrollToPosition(monthlyList.size - 1)
        binding.rvWeekMonth.adapter = monthlyAdapter
        monthlyAdapter!!.updateList(monthlyList)
    ...
    private fun registerObservers() {
        ...
        viewModel.stepsHistoryBetweenList.observe(viewLifecycleOwner) { data ->
            if (data != null) {
                //Utilities.printData("MonthlyApiHistory",data.stepGoalHistory,true)
    ...
    override fun onMonthlyItemSelection(position: Int, month: MonthModel) {
    ...
    private fun showFitnessData(weeklyHistoryFromApi: List<FitnessEntity.StepGoalHistory>) {
        try {
            val monthlyFitnessDataList: MutableList<FitnessEntity.StepGoalHistory> = mutableListOf()
            val sd: DateFormat =
                SimpleDateFormat(DateHelper.SERVER_DATE_YYYYMMDD, Locale.getDefault())
            val fromDate = sd.parse(startDate)

            val month: String = DateHelper.getMonthAsStringMMMFromDate(fromDate!!)

            val numberOfDaysOfMonth = DateHelper.getNumberOfDaysOfMonthFromDate(
                startDate,
                DateHelper.SERVER_DATE_YYYYMMDD
            )
            var dataMap: FitnessEntity.StepGoalHistory

            if (weeklyHistoryFromApi.isNotEmpty() && weeklyHistoryFromApi.any { it.stepsCount != 0 }) {
                for (i in 0 until numberOfDaysOfMonth) {
                    ...
                    dataMap = FitnessEntity.StepGoalHistory(lastRefreshed = Date())
                    dataMap.stepID = 0
                    dataMap.goalID = 0
                    ...
                    monthlyFitnessDataList.add(dataMap)
                }
            }

            /*            ArrayList<StepGoalHistory> filteredList = new ArrayList<>();
            for ( StepGoalHistory history : monthlyFitnessDataList ) {
                if ( DateHelper.stringDateToDate(DateHelper.SHORT_DATEFORMAT_YYYYMMDD, history.getRecordDate()).before(new Date()) ) {
                    filteredList.add(history);
                }
            }*/
            val filteredList: MutableList<FitnessEntity.StepGoalHistory> = mutableListOf()
            for (history in monthlyFitnessDataList) {
                if (DateHelper.formatDateValue(
            ...
            monthlyDataList.addAll(filteredList)
            ...
            if (monthlyDataList.isNotEmpty()) {
                ...
                /*                val filteredList: ArrayList<FitnessEntity.StepGoalHistory> = ArrayList<FitnessEntity.StepGoalHistory>()
                                for (history in monthlyDataList) {
                                    if (DateHelper.stringDateToDate(DateHelper.SERVER_DATE_YYYYMMDD, history.recordDate).before(Date())) {
                                        filteredList.add(history)
                                    }
                                }*/
                binding.rvDailyData.layoutAnimation = animation
                dailyDataAdapter!!.updateList(monthlyDataList)
                binding.rvDailyData.scheduleLayoutAnimation()
            } else {
    ...
    private fun setGraphData() {
        try {
            binding.graphMonthly.clear()
            binding.graphMonthly.invalidate()
            if (monthlyDataList.isNotEmpty()) {
                binding.graphMonthly.visibility = View.VISIBLE
                binding.graphUnit.visibility = View.VISIBLE
                val yAxis: YAxis = binding.graphMonthly.axisLeft
                yAxis.setDrawTopYLabelEntry(true)
                yAxis.setDrawZeroLine(false)
                yAxis.granularity = 1f
                yAxis.axisMinimum = 0f
                val xAxis: XAxis = binding.graphMonthly.xAxis
                ...
                xAxis.valueFormatter = object : IAxisValueFormatter {
                    override fun getFormattedValue(value: Float, axis: AxisBase?): String? {
                        for (i in 1..monthlyDataList.size) {
                            if (value == i.toFloat()) {
                                //return DateHelper.formatDateValue("dd MMM", monthlyDataList[i - 1].recordDate)
                                return DateHelper.formatDateValue(
                                    "dd",
                                    monthlyDataList[i - 1].recordDate
                                )
                            }
                        }
                        return ""
                ...
                when (type) {
                    Constants.STEPS_COUNT -> {
                        var i = 0
                        while (i < monthlyDataList.size) {
                            if (Utilities.isNullOrEmptyOrZero(monthlyDataList[i].stepsCount.toString())) {
                                entries.add(BarEntry((i + 1).toFloat(), 0.0f))
                            } else {
                                entries.add(
                                    BarEntry(
                                        (i + 1).toFloat(),
                                        monthlyDataList[i].stepsCount.toFloat()
                                    )
                                )
                            }
                            i++
                    ...
                    Constants.DISTANCE -> {
                        var i = 0
                        while (i < monthlyDataList.size) {
                            if (Utilities.isNullOrEmptyOrZero(monthlyDataList[i].distance.toString())) {
                                entries.add(BarEntry((i + 1).toFloat(), 0.0f))
                            } else {
                                entries.add(
                                    BarEntry(
                                        (i + 1).toFloat(),
                                        fitnessHelper.convertMtrToKmsValueNew(monthlyDataList[i].distance.toString())
                                            .toFloat()
                                    )
                                )
                            }
                    ...
                    Constants.CALORIES -> {
                        var i = 0
                        while (i < monthlyDataList.size) {
                            if (Utilities.isNullOrEmptyOrZero(monthlyDataList[i].calories.toString())) {
                                entries.add(BarEntry((i + 1).toFloat(), 0.0f))
                            } else {
                                entries.add(
                                    BarEntry(
                                        (i + 1).toFloat(),
                                        monthlyDataList[i].calories.toFloat()
                                    )
                                )
                            }
                            i++
                ...
                binding.graphMonthly.isDoubleTapToZoomEnabled = false
                binding.graphMonthly.setTouchEnabled(false)
                binding.graphMonthly.isHighlightPerTapEnabled = false
                binding.graphMonthly.setTouchEnabled(false)
                binding.graphMonthly.isDragEnabled = false
                binding.graphMonthly.setPinchZoom(false)
                binding.graphMonthly.isHighlightPerTapEnabled = false
                binding.graphMonthly.setDrawValueAboveBar(false)
                //binding.graphMonthly.setVisibleXRangeMaximum(7f)
                binding.graphMonthly.setVisibleXRangeMaximum((daysCount + 1).toFloat())
                binding.graphMonthly.invalidate()
                //binding.graphMonthly.animateY(1200);
                binding.graphMonthly.animateXY(1000, 1000)
            } else {
                binding.graphUnit.visibility = View.GONE
                binding.graphMonthly.invalidate()
                //binding.graphMonthly.clearValues();
                binding.graphMonthly.notifyDataSetChanged()
                binding.graphMonthly.setNoDataText(resources.getString(R.string.NO_HISTORY_AVAILABLE))
                binding.graphMonthly.setNoDataTextColor(
                    ContextCompat.getColor(
                        requireContext(),
                        R.color.textViewColorBlack
                    )
                )
                val paint: Paint = binding.graphMonthly.getPaint(BarChart.PAINT_INFO)
    ...
    /*    private fun getFitnessData(startDate: Date, endDate: Date): MutableList<FitnessEntity.StepGoalHistory> {
            val fitnessDataList: MutableList<FitnessEntity.StepGoalHistory> = mutableListOf()
            try {
                //val fitnessListSize = DateHelper.getDateDifferenceInDays(startDate,endDate) + 1
                ...
                when (numberOfDays) {
                    28 -> newEndDate.setDate(startDate.date + 27)
                    29 -> newEndDate.setDate(startDate.date + 28)
                    30 -> newEndDate.setDate(startDate.date + 29)
                    31 -> newEndDate.setDate(startDate.date + 30)
                }
                ...
                fitnessDataManager!!.readHistoryData(startDate, newEndDate).addOnCompleteListener {
                    ...
                    if (fitnessDataJsonArray != null) {
                        //Utilities.printLogError("Google Fit data for $numberOfDays days---> $fitnessDataJsonArray")
    ...
    *//*                                            dataMap.distance = (fitnessDataJsonArray.getJSONObject(j).getString(
                    ...
                    monthlyDataList.clear()
                    monthlyDataList.addAll(fitnessDataList)
                    monthlyDataList.sortBy { it.recordDate }
                    Utilities.printData("monthlyDataList", monthlyDataList)
                    calculateTotalData()
                    setGraphData()

                    val filteredList = fitnessDataList.filter { DateHelper.stringDateToDate(DateHelper.SERVER_DATE_YYYYMMDD, it.recordDate) < Date()
                    }.toMutableList()
                    binding.rvDailyData.layoutAnimation = animation
    ...
    /*    private fun getCustomizedList(type: String): MutableList<FitnessEntity.StepGoalHistory> {
            ...
            try {
                ...
                var distance = 0.0
                var calories = 0
                var item: FitnessEntity.StepGoalHistory

                val calendar = Calendar.getInstance()
                calendar.time = DateHelper.stringDateToDate(DateHelper.SERVER_DATE_YYYYMMDD, monthlyDataList[0].recordDate)
                val numberOfDays = calendar.getActualMaximum(Calendar.DAY_OF_MONTH)

                when (type) {
                    Constants.STEPS_COUNT -> {
                        for (i in 0 until 7) {
                            item = FitnessEntity.StepGoalHistory(lastRefreshed = Date())
                            if (n <= numberOfDays - 1 && n <= monthlyDataList.size - 1) {
                                item.recordDate = monthlyDataList[n].recordDate
                                stepCount = monthlyDataList[n].stepsCount
                            }
                            item.stepsCount = stepCount
                            list.add(item)
                            when (n) {
                                0 -> {
                                    n += 4
                    ...
                    Constants.DISTANCE -> {
                        for (i in 0 until 7) {
                            ...
                            if (n <= numberOfDays - 1 && n <= monthlyDataList.size - 1) {
                                item.recordDate = monthlyDataList[n].recordDate
                                distance = monthlyDataList[n].distance
                            }
                    ...
                    Constants.CALORIES -> {
                        for (i in 0 until 7) {
                            ...
                            if (n <= numberOfDays - 1 && n <= monthlyDataList.size - 1) {
                                item.recordDate = monthlyDataList[n].recordDate
                                calories = monthlyDataList[n].calories
                            }
    ...
    /*    private fun setGraphData() {
            try {
                val customizedMonthlyList = getCustomizedList(type)
                if (!customizedMonthlyList.isNullOrEmpty()) {
                    binding.graphMonthly.visibility = View.VISIBLE
                    binding.graphUnit.visibility = View.VISIBLE

                    binding.graphMonthly.description.text = ""
                    binding.graphMonthly.legend.isEnabled = false
                    binding.graphMonthly.axisRight.isEnabled = false
                    binding.graphMonthly.isDoubleTapToZoomEnabled = false
                    binding.graphMonthly.setTouchEnabled(false)
                    ...
                    xAxis.valueFormatter = object : IAxisValueFormatter {
                        override fun getFormattedValue(value: Float, axis: AxisBase?): String? {
                            for (i in 1..customizedMonthlyList.size) {
                                if (value == i.toFloat()) {
                                    return DateHelper.formatDateValue("dd MMM", customizedMonthlyList[i - 1].recordDate)
                                }
                            }
                            return ""
                        }
                    ...
                    when (type) {
                        Constants.STEPS_COUNT -> {
                            for (i in customizedMonthlyList.indices) {
                                if (Utilities.isNullOrEmptyOrZero(customizedMonthlyList[i].stepsCount.toString())) {
                                    entries.add(Entry((i + 1).toFloat(), 0f))
                                } else {
                                    entries.add(Entry((i + 1).toFloat(), customizedMonthlyList[i].stepsCount.toFloat()))
                                }
                            }
                        }
                        Constants.DISTANCE -> {
                            for (i in customizedMonthlyList.indices) {
                                if (Utilities.isNullOrEmptyOrZero(customizedMonthlyList[i].distance.toString())) {
                                    entries.add(Entry((i + 1).toFloat(), 0f))
                                } else {
                                    entries.add(Entry((i + 1).toFloat(), customizedMonthlyList[i].distance.toFloat()))
                                }
                            }
                        }
                        Constants.CALORIES -> {
                            for (i in customizedMonthlyList.indices) {
                                if (Utilities.isNullOrEmptyOrZero(customizedMonthlyList[i].calories.toString())) {
                                    entries.add(Entry((i + 1).toFloat(), 0f))
                                } else {
                                    entries.add(Entry((i + 1).toFloat(), customizedMonthlyList[i].calories.toFloat()))
                                }
                            }
                        }
                    }
                    ...
                    binding.graphMonthly.data = lineData
                    ...
                    binding.graphMonthly.invalidate()
                    //binding.graphMonthly.animateY(1200)
                    binding.graphMonthly.animateXY(1500, 1500)
                } else {
                    binding.graphUnit.visibility = View.GONE
                    binding.graphMonthly.invalidate()
                    //binding.graphMonthly.clearValues();
                    binding.graphMonthly.notifyDataSetChanged()
                    binding.graphMonthly.setNoDataText(resources.getString(R.string.NO_HISTORY_AVAILABLE))
                    binding.graphMonthly.setNoDataTextColor(ContextCompat.getColor(requireContext(), R.color.textViewColorBlack))
                    val paint: Paint = binding.graphMonthly.getPaint(BarChart.PAINT_INFO)
