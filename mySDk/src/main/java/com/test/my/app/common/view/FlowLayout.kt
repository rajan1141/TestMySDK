...
class FlowLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    ...
    private var mFlow: Boolean = DEFAULT_FLOW
    private var mChildSpacing: Int = DEFAULT_CHILD_SPACING
    private var mMinChildSpacing: Int = DEFAULT_CHILD_SPACING
    private var mChildSpacingForLastRow: Int = DEFAULT_CHILD_SPACING_FOR_LAST_ROW
    private var mRowSpacing: Float = DEFAULT_ROW_SPACING
    private var mAdjustedRowSpacing: Float = DEFAULT_ROW_SPACING
    private var mRtl: Boolean = DEFAULT_RTL
    private var mMaxRows: Int = DEFAULT_MAX_ROWS
    private var mGravity: Int = UNSPECIFIED_GRAVITY
    private var mRowVerticalGravity: Int = ROW_VERTICAL_GRAVITY_AUTO
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (mChildSpacingForLastRow == SPACING_ALIGN) {
            ...
            mHorizontalSpacingForRow.add(
                getSpacingForRow(
                    mChildSpacingForLastRow, rowSize,
                    rowTotalChildWidth, childNumInRow
                )
            )
        } else {
        ...
        val rowSpacing: Float =
            if (mRowSpacing == SPACING_AUTO.toFloat() && heightMode == MeasureSpec.UNSPECIFIED) 0.0f
            else mRowSpacing
        if (rowSpacing == Companion.SPACING_AUTO.toFloat()) {
            ...
            if (rowNum > 1) {
                measuredHeight =
                    if (heightMode == MeasureSpec.UNSPECIFIED) (measuredHeight + mAdjustedRowSpacing * (rowNum - 1)).toInt() else Math.min(
    ...
    companion object {
        private val LOG_TAG: String = FlowLayout::class.java.simpleName
        ...
        private val DEFAULT_CHILD_SPACING_FOR_LAST_ROW: Int = SPACING_UNDEFINED
